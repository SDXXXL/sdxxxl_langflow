...
<CodeGroup>
  ```yaml Helm (using an existing Secret) theme={null}
  redis:
    external:
      enabled: true
  ...
  ```yaml Kubernetes Secret theme={null}
  apiVersion: v1
  kind: Secret
  metadata:
...
<CodeGroup>
  ```yaml Helm (inline values) theme={null}
  redis:
    external:
      enabled: true
      cluster:
...
<CodeGroup>
  ```yaml Helm (Standalone - server TLS) theme={null}
  config:
    customCa:
  ...
  ```yaml Helm (Cluster - server TLS) theme={null}
  config:
    customCa:
      secretName: "langsmith-custom-ca"  # Secret containing your CA bundle
      secretKey: "ca.crt"    # Key in the Secret with the CA bundle
  ...
  ```yaml Kubernetes Secret (CA bundle) theme={null}
  apiVersion: v1
  kind: Secret
  metadata:
...
<CodeGroup>
  ```yaml Helm (client Auth) theme={null}
  redis:
    external:
      enabled: true
      clientCert:
  ...
  ```yaml Kubernetes Secret (client cert/key) theme={null}
  apiVersion: v1
  kind: Secret
  metadata:
