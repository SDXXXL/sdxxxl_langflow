...
````yaml https://api.host.langchain.com/openapi.json delete /v2/sandboxes/boxes/{name}
openapi: 3.1.0
info:
...
paths:
  /v2/sandboxes/boxes/{name}:
    delete:
      ...
      description: Delete a SandboxClaim from tenant's namespace.
      operationId: delete_claim_v2_sandboxes_boxes__name__delete
      parameters:
        - name: name
          in: path
          required: true
