...
````yaml https://api.host.langchain.com/openapi.json get /v2/sandboxes/templates/{name}
openapi: 3.1.0
info:
...
paths:
  /v2/sandboxes/templates/{name}:
    get:
      tags:
        - Sandboxes (v2)
      summary: Get a SandboxTemplate
      description: |-
        Get a specific SandboxTemplate by name in tenant's namespace.

        This endpoint queries the database for fast performance.
      operationId: get_template_v2_sandboxes_templates__name__get
      parameters:
        - name: name
          in: path
          required: true
