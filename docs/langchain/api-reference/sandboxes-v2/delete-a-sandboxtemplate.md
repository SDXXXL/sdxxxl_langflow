...
````yaml https://api.host.langchain.com/openapi.json delete /v2/sandboxes/templates/{name}
openapi: 3.1.0
info:
...
paths:
  /v2/sandboxes/templates/{name}:
    delete:
      tags:
        - Sandboxes (v2)
      summary: Delete a SandboxTemplate
      description: |-
        Delete a SandboxTemplate from tenant's namespace.

        Deletion will fail if the template is in use by any sandboxes or pools.
      operationId: delete_template_v2_sandboxes_templates__name__delete
      parameters:
        - name: name
          in: path
          required: true
