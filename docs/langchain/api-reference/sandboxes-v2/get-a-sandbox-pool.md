...
````yaml https://api.host.langchain.com/openapi.json get /v2/sandboxes/pools/{name}
openapi: 3.1.0
info:
...
paths:
  /v2/sandboxes/pools/{name}:
    get:
      tags:
        - Sandboxes (v2)
      summary: Get a Sandbox Pool
      description: |-
        Get a specific Sandbox Pool by name in tenant's namespace.

        This endpoint queries the database for fast performance.
      operationId: get_warmpool_v2_sandboxes_pools__name__get
      parameters:
        - name: name
          in: path
          required: true
