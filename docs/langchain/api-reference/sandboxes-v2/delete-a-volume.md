...
````yaml https://api.host.langchain.com/openapi.json delete /v2/sandboxes/volumes/{name}
openapi: 3.1.0
info:
  ...
  /v2/sandboxes/volumes/{name}:
    delete:
      ...
      description: |-
        ...
        This will fail if any templates reference this volume. Delete or update
        those templates first before deleting the volume.
      operationId: delete_volume_v2_sandboxes_volumes__name__delete
      parameters:
        - name: name
          in: path
          required: true
      ...
      responses:
        ...
        '409':
          description: Conflict - Volume is in use by templates
          content:
            application/json:
              example:
                detail:
                  error: VolumeInUse
                  message: >-
                    Volume 'my-vol' cannot be deleted: referenced by templates:
                    my-template-1, my-template-2
        '422':
