...
````yaml https://api.host.langchain.com/openapi.json delete /v2/sandboxes/pools/{name}
openapi: 3.1.0
info:
...
paths:
  /v2/sandboxes/pools/{name}:
    delete:
      tags:
        - Sandboxes (v2)
      summary: Delete a Sandbox Pool
      description: |-
        Delete a Sandbox Pool from tenant's namespace.

        This will terminate all sandboxes in the pool.
      operationId: delete_warmpool_v2_sandboxes_pools__name__delete
      parameters:
        - name: name
          in: path
          required: true
