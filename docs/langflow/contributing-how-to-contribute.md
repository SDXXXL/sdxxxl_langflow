# Contribute to Langflow | Langflow Documentation

- Contribute
- Contribute to Langflow

On this page# Contribute to Langflow
...
- uv version 0.4 or later
- Node.js
- Make (Linux and macOS only)
...
`_10git remote add FORK_NAME https://github.com/GIT_USERNAME/langflow.git`Replace the following:
...
Run from source on macOS/LinuxIn your terminal, navigate to the root of the Langflow directory, and then run make run_cli.
...
Run from source with Windows CMDTo run Langflow from source on Windows, you can use the Langflow repository's included scripts, or run the commands in the terminal.
...
Run from source with PowerShellTo run Langflow from source on Windows, you can use the Langflow repository's included scripts, or run the commands in the terminal.
...
tipRun make help to display all available make commands.
...
_10# Run backend in development mode (includes hot reload)_10make backend
_10# In another terminal, run frontend in development mode (includes hot reload)_10make frontend
...
`_10make run_clic`This command cleans the build cache and rebuilds everything from scratch, which resolves most frontend-related issues when switching between versions.
...
For style guidance, see the Google Developer Documentation Style Guide.

1. Install Node.js.
2. Fork the Langflow GitHub repository.
